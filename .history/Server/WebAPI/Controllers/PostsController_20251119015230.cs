...
public class PostsController : ControllerBase
{
    ...
    public async Task<ActionResult<PostDto>> Create([FromBody] CreatePostDto request)
    {
        ...
        var created = await _posts.AddAsync(new Post { Title = request.Title, Body = request.Body, UserId = request.UserId, LikedByUserIds = new List<int>() });

        var dto = new PostDto { Id = created.Id, Title = created.Title, Body = created.Body, UserId = created.UserId, LikesCount = created.LikedByUserIds.Count, LikedByUserIds = created.LikedByUserIds};
    ...
    public async Task<ActionResult<PostDto>> GetSingle(int id)
    {
        ...
        return Ok(new PostDto { Id = p.Id, Title = p.Title, Body = p.Body, UserId = p.UserId, LikesCount = p.LikedByUserIds?.Count ?? 0, LikedByUserIds = p.LikedByUserIds ?? new List<int>() });
    }
        ...
        [FromQuery] int? userId)
    {
        ...
        var result = q.Select(p => new PostDto { Id = p.Id, Title = p.Title, Body = p.Body, UserId = p.UserId, LikesCount = p.LikedByUserIds!.Count, LikedByUserIds = p.LikedByUserIds! })
                      .ToList();
        ...
        [FromQuery] int? userId = null)
    {
        ...
        var items = q.Skip((page - 1) * pageSize)
                    ...
                    {
                         Id = p.Id, Title = p.Title, Body = p.Body, UserId = p.UserId, LikesCount = p.LikedByUserIds!.Count, LikedByUserIds = p.LikedByUserIds!
                    })
                    .ToList();
    ...
    public async Task<ActionResult<PostDto>> Vote(int id, [FromQuery] int userId)
    {
        ...
        var dto = new PostDto
        {
            ...
            Body = post.Body,
            UserId = post.UserId,
            LikesCount = post.LikedByUserIds?.Count ?? 0, LikedByUserIds = post.LikedByUserIds ?? new List<int>()
        };
