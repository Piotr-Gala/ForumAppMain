...
public class PostsController : ControllerBase
{
    ...
    // GET /Posts?query=abc
    [HttpGet]
    public ActionResult<IEnumerable<PostDto>> GetMany([FromQuery] string? query)
    {
        var q = _posts.GetManyAsync(); // IQueryable<Post>

        if (!string.IsNullOrWhiteSpace(query))  // filtering by title
    ...
    public async Task<ActionResult<PostDto>> Update(int id, [FromBody] UpdatePostDto request)
    {
        ...
        return Ok(new PostDto { Id = p.Id, Title = p.Title, Body = p.Body });
    }
