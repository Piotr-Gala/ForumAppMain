...
public class PostsController : ControllerBase
{
    ...
    public async Task<ActionResult<PostDto>> Create([FromBody] CreatePostDto request)
    {
        ...
        var created = await _posts.AddAsync(new Post { Title = request.Title, Body = request.Body, UserId = request.UserId, LikesCount = 0 });
    ...
    public async Task<ActionResult<PostDto>> Update(int id, [FromBody] UpdatePostDto request)
    {
        ...
        return Ok(new PostDto { Id = p.Id, Title = p.Title, Body = p.Body, UserId = p.UserId, LikesCount });
    }
    ...
    public async Task<ActionResult<PostDto>> Vote(int id, [FromQuery] int delta = 1)
    {
        var post = await _posts.GetSingleAsync(id);

        // prosta logika – możemy później ograniczyć zakres, jak będziesz chciał
