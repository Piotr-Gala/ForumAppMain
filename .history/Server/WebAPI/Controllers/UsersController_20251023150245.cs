...
public class UsersController : ControllerBase
{
    ...
    {
        // walidację (puste pola) zrobi też repo i rzuci ValidationException -> 400 przez middleware
    ...
    {
        var u = await _users.GetSingleAsync(id); // 404 jeśli brak (middleware)
    ...
    {
        var u = await _users.GetSingleAsync(id); // 404 jeśli nie istnieje

        u.Username = request.UserName;
        if (!string.IsNullOrWhiteSpace(request.Password))
            u.Password = request.Password;

        await _users.UpdateAsync(u); // może rzucić DuplicateEntityException -> 409
        return Ok(new UserDto { Id = u.Id, UserName = u.Username });
    }
    ...
    {
        await _users.DeleteAsync(id); // 404 jeśli brak
        return NoContent();
    }
}
